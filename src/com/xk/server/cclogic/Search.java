...
public class Search {
	...
	private class SortItem {
		...
		SortItem(int mvHash) {
			...
			mvs = new Integer[MAX_GEN_MOVES];
			vls = new Integer[MAX_GEN_MOVES];
			moves = 0;
			 Integer[] mvsAll = pos.generateAllMoves();
			 int numAll=mvsAll.length;
	...
	private int searchQuiesc(int vlAlpha_, int vlBeta) {
		...
		if (pos.inCheck()) {
			mvs = pos.generateAllMoves();
			genMoves=mvs.length;
			Integer[] vls = new Integer[MAX_GEN_MOVES];
